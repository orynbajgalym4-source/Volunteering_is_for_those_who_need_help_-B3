...
export async function POST(request: Request) {
  ...
  const webAppUrl = isInvite ? `${origin}/?startapp=${encodeURIComponent(startParam)}` : origin;
  ...
  await telegramBotCall("sendMessage", {
    chat_id: message.chat.id,
    text,
    reply_markup: { inline_keyboard: [[{ text: isInvite ? "Открыть приглашение" : "Открыть Asar", web_app: { url: webAppUrl } }]] },
  });
