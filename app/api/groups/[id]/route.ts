...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const db = database();
  const membership = await db.prepare("SELECT role FROM group_members WHERE group_id = ? AND member_key = ? AND membership_source = 'EXPLICIT'").bind(id, owner.email).first<{ role: "OWNER" | "MEMBER" }>();
  ...
  const memberRows = await db.prepare("SELECT id, member_key, display_name, username, role, joined_at FROM group_members WHERE group_id = ? AND membership_source = 'EXPLICIT' ORDER BY role = 'OWNER' DESC, joined_at")
  ...
  const members: GroupMemberView[] = await Promise.all(memberRows.results.map(async (row) => {
    const count = await db.prepare(`SELECT COUNT(DISTINCT a.id) AS total FROM asars a
      ...
      .bind(id, row.member_key, row.member_key, row.id).first<{ total: number }>();
