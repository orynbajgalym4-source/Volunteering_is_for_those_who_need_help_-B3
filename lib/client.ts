import { ensureTelegramSession, getTelegramInitData, waitForTelegramInitData } from "./telegram";
...
export async function api<T>(url: string, options?: RequestInit): Promise<T> {
  ...
  if (protectedRequest && initData) await ensureTelegramSession(initData).catch(() => undefined);
