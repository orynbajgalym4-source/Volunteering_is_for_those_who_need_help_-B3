...
export function AsarDetail({ id }: { id: string }) {
  ...
  return <div className="app-page"><AppHeader title="Панель асара" /><main className="app-main">
    ...
    <div className="detail-grid">
      <section className="hero-panel"><div className="hero-panel-top"><StatusBadge state={asar.lifecycleStatus} /><span className="muted">{formatDate(asar.startsAt)}</span></div><h1>{asar.title}</h1><p className="location">{asar.publicLocation} · организатор: {asar.ownerName}</p>
    ...
    <section className="section-block"><div className="section-heading"><h2>Люди и ресурсы</h2>{missing.length > 0 && asar.lifecycleStatus !== "DRAFT" && <Link className="button button-danger" href={`/app/asars/${id}/share`}>Поделиться нехваткой</Link>}</div>
      <div className="requirement-list">{asar.requirements.map((item) => <article className="requirement-card" key={item.id}><div className="requirement-head"><span className="requirement-icon">{item.kind === "PERSON" ? "●" : "◆"}</span><div className="requirement-copy"><h3>{item.title} {item.isCritical && <span className="danger-text">*</span>}</h3><p>{item.description || (item.kind === "PERSON" ? "Человеческая роль" : "Необходимый ресурс")}</p></div><div className="requirement-numbers"><strong>{item.confirmedQuantity} / {item.requiredQuantity}</strong><small>подтверждено</small></div></div><div className="bar"><i style={{ width: `${Math.min(100, item.confirmedQuantity / item.requiredQuantity * 100)}%` }} /></div>
