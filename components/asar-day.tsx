...
export function AsarDay({ id }: { id: string }) {
  ...
  const commitments = asar.requirements.flatMap((requirement) => (requirement.commitments ?? []).map((commitment) => ({ ...commitment, requirementTitle: requirement.title, kind: requirement.kind })));
