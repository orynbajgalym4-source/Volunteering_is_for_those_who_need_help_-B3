...
function AsarHistoryCard({ asar }: { asar: AsarView }) {
  ...
  return <Link className="asar-card circle-asar-card" href={`/app/asars/${asar.id}`}><div className="asar-card-top"><StatusBadge state={asar.lifecycleStatus} /><span className="muted">{formatAsarSchedule(asar.startsAt, asar.timeMode)}</span></div><h2>{asar.title}</h2><p>{asar.publicLocation || "Место уточняется"}</p>{outcome && <small className="history-outcome">{outcome}</small>}</Link>;
}
...
export function GroupDetail({ id }: { id: string }) {
  ...
  return <div className="app-page"><AppHeader title="Круг" /><main className="app-main"><div className="page-heading compact-heading"><Link className="text-link" href="/app/profile">← Профиль</Link></div><section className="group-hero"><GroupAvatar group={group} size="large" /><div><span className="section-kicker">{group.role === "OWNER" ? "Вы организатор" : "Вы участник"}</span><h1>{group.name}</h1><p>{group.description || "Постоянный круг людей для общих дел."}</p><div className="group-stats"><span><strong>{group.memberCount}</strong> участников</span><span><strong>{group.asarCount}</strong> дел</span></div></div></section><div className="group-primary-action"><Link className="button button-primary" href={`/app/asars/new?group=${group.id}`}>+ Создать асар в круге</Link></div>
    <section className="section-block"><div className="section-heading"><div><span className="section-kicker">Люди</span><h2>Участники круга</h2></div></div><div className="group-members">{group.members.map((member) => <Link href={member.id === group.currentMemberId ? `/app/profile?circle=${id}` : `/app/groups/${id}/members/${member.id}`} className="group-member-card member-profile-card" key={member.id}><span className="member-avatar">{member.displayName.slice(0, 1).toUpperCase()}</span><span className="member-card-main"><strong>{member.displayName}{member.id === group.currentMemberId ? " · Вы" : ""}</strong><small>{member.role === "OWNER" ? "Организатор" : "Участник"} · {member.completedAsarCount} дел</small>{member.canViewInvitationRecency && <em>{invitedAgo(member.lastInvitedAt)}</em>}</span><i>›</i></Link>)}</div></section>
