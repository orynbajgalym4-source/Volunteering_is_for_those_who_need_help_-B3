...
export function GroupCard({ group, selected, onSelect }: { group: GroupSummary; selected?: boolean; onSelect?: () => void }) {
  const content = <><GroupAvatar group={group} /><span className="group-card-copy"><strong>{group.name}</strong><small>{group.memberCount} участников · {group.asarCount} дел</small>{group.description && <small>{group.description}</small>}{group.myOffers && <OfferChips offers={group.myOffers} limit={2} />}</span><span className="group-card-arrow">{selected ? "✓" : "›"}</span></>;
