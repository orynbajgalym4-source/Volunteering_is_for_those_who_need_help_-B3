...
export function OrganizerDashboard({ embedded = false, profile }: { embedded?: boolean; profile?: TelegramProfile | null }) {
  ...
  return <div className={embedded ? "app-page tg-dashboard" : "app-page"}>
    ...
    <main className="app-main">
      ...
      {loading ? <LoadingCard /> : error ? <EmptyState title="Откройте Asar в Telegram" text={error} action={<a className="button button-primary" href="https://t.me/asar_ops_bot">Перейти к боту</a>} /> : visible.length === 0 ? <EmptyState title={items.length ? "Здесь пока пусто" : "Начните с первого асара"} text="Опишите общее дело, добавьте критические роли и получите приглашение для Telegram." action={<Link className="button button-primary" href="/app/asars/new">Собрать асар</Link>} /> :
