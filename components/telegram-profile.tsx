...
export function TelegramProfilePage() {
  ...
  return <div className="app-page"><AppHeader title="Профиль" /><main className="app-main wizard-shell"><div className="profile-identity"><div className="tg-profile-avatar">{profile?.firstName?.[0] ?? "А"}</div><div><span className="section-kicker">Telegram‑профиль</span><h1>{profile ? `${profile.firstName}${profile.lastName ? ` ${profile.lastName}` : ""}` : "Инициатор"}</h1><p>{profile?.username ? `@${profile.username}` : profile ? `Telegram ID ${profile.id}` : "Данные появятся внутри Telegram"}</p></div></div>
    <section className="profile-section"><div className="section-heading"><div><span className="section-kicker">Ваши круги</span><h2>Круги</h2></div><Link className="button button-secondary" href="/app/groups/new">+ Создать</Link></div>{groupsLoading ? <div className="loading-inline">Загружаем круги…</div> : groups.length ? <><p className="profile-section-intro">В каждом круге можно отдельно указать, с чем к вам разрешено обращаться.</p><div className="group-choice-list">{groups.map((group) => <GroupCard group={group} key={group.id} />)}</div></> : <div className="group-empty compact"><span>А</span><h3>Кругов пока нет</h3><p>Создайте постоянный круг людей для будущих асаров.</p><Link className="button button-primary" href="/app/groups/new">Создать первый круг</Link></div>}</section>
