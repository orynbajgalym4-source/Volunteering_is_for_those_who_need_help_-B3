...
2. Кто подтвердил участие и что фактически произошло в день асара.
...
5. После подтверждения увидеть точный адрес и при необходимости отменить участие до начала.
6. Если инициатор запустил перекличку, отдельно ответить по каждой занятой роли.
7. После завершения увидеть состоявшийся асар в своей истории, если участие отмечено как `ATTENDED`.
...
В production обычный organizer-интерфейс открывается через Telegram. Публичные token-ссылки продолжают работать в браузере без Telegram-аккаунта.
...
Ссылка действует 14 дней, если асар не завершён и не отменён. Интерфейс умеет подготовить текст, Telegram deep link, карточку-изображение и системный Web Share.
...
- Telegram-сообщение отправляется только при явном opt-in; остальным создаётся ротируемая ручная ссылка.
...
До назначенного времени асар можно начать раньше только при `READY`. После наступления времени инициатор может начать проведение и открыть чек-лист участников.
...
Доступны варианты: совет, знакомство с человеком, местная информация, еда или чай, помещение, обзвон, координация, инструмент и передача опыта. `RECEIVE_ONLY` означает, что человеку сейчас самому нужна поддержка или он не принимает новые просьбы.
...
В проекте работают три независимые машины состояний.
...
- терминальный асар нельзя редактировать или снова открыть;
